...
@app.route('/api/cpubound_task', methods=['POST'])
def cpuboundtask():
    job = task_queue.enqueue('app.tasks.example',
                             on_success=rq.Callback(report_success),
                             htmltext='some html text will be here')
    job_id = job.get_id()
    return jsonify("/api/call POST", job_id)
...
def cpu_bound_prime_numbers():
    ...
    for num in range(1000, 4000):  # i7-8700k deals with it in  12.82s with debug range(1000, 16000)
